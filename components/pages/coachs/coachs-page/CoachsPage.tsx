...
function CoachsCard({ data, isRTL }: { data: TrainerCardData; isRTL: boolean }) {
  ...
  return (
    <article
      className="group relative overflow-hidden rounded-2xl border border-border bg-card shadow-[0_6px_20px_rgba(0,0,0,0.12)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_14px_32px_rgba(34,95,236,0.22)]"
      dir={isRTL ? "rtl" : "ltr"}
    >
      <div className="relative h-56 w-full overflow-hidden border-b border-border bg-muted/50 sm:h-64">
        <Image
          src={DEFAULT_AVATAR}
          alt={title}
          fill
          ...
          <div dir="ltr" className={`flex items-center gap-2 ${isRTL ? "flex-row-reverse" : "flex-row"}`}>
            <div className="flex shrink-0 items-center">
              {Array.from({ length: 4 }).map((_, index) => (
                <div
                  key={index}
                  className="relative size-7 overflow-hidden rounded-full border-2 border-card shadow-[0_2px_6px_rgba(0,0,0,0.22)]"
                  style={{
    ...
    <div className="mx-auto w-full max-w-7xl px-4 py-6 md:px-6">
      ...
      <section className="rounded-3xl border border-border bg-card p-4 md:p-6">
        ...
        <div className="grid grid-cols-[repeat(auto-fit,minmax(240px,1fr))] gap-4">
          {COACHS.map((coach) => (
            <CoachsCard key={coach.id} data={coach} isRTL={isRTL} />
          ))}
        </div>
      </section>
    </div>
